 # 代码评审 Agent（Java + React）

- 目标：把“代码评审”做成产品闭环——提交代码或 git diff → 结构化评审 → 将评审结果作为上下文继续对话，产出可执行修改方案与关键代码片段
- 部署：后端 Spring Boot（systemd）+ 前端 React（Nginx 静态），GitHub Actions 产物构建
...
- SDK（轻量 Agent / Tool / Chain / MCP）
  - Agent 核心：最小 tool-calling 循环（模型输出约定块触发工具执行，记录 scratchpad）
...
- Agent/SDK：Java 轻量实现（参考 langchain4j 思想，但未直接依赖）
- 大模型：ChatGLM（HTTP API）
- 基础设施：Nginx（静态+反代）、systemd（后端常驻）
- CI/CD：GitHub Actions（Artifacts 构建并下载）

模块分层
- 前端：React + Vite，单页页面，两个面板（Code Review、Chat），静态资源由 Nginx托管
  - 页面入口： App.jsx
  - 样式与响应式布局： styles.css
  - 前端接口封装： api.js
...
- SDK：轻量 Agent / Chain / RAG / MCP 抽象，统一模型通道
  - Agent核心（tool-calling）： SimpleAgent
